...
for i1=1:size(point,1)
    ...
    for j1=2:(size(index,2)-1)
        ...
        %考虑距离权重
%{        
        for j22=(j1+1):size(index,2)
            ii=1;
            rr(1,ii)=norm(point(index(i1,j1),:)-point(index(i1,j2),:));
            ii=ii+1;
        end
        r=max(rr(1,:))/2;
 %}
        %if norm(point(index(i1,j1),:)-point(index(i1,j2),:))<0.5
            wjk=exp(-norm(point(index(i1,j1),:)-point(index(i1,j2),:))^2/r^2);
        %else 
        %    wjk=0;
        %end
        st=st+(wjk+ddjk)*pk*pk';
        %st=st+pk*pk';
        end
    end
        [v,d]=eig(st);
       ...
       %arrow3([point(i1,1) point(i1,2) point(i1,3)],[n(1,1) n(1,2) n(1,3)],'b',0.5)
end
toc
